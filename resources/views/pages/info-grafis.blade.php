...
@section('title', 'Info Grafis | Desa Sambo')
...
@section('content')
    @php
        ...
        $apbdesYears = collect($apbdesStats['years'] ?? []);
        $apbdesChartMax = max(1, (int) $apbdesYears->flatMap(fn ($year) => [$year['income'], $year['spending']])->max());
        $formatRupiah = fn ($amount) => 'Rp'.number_format((int) $amount, 0, ',', '.');
        $formatShortRupiah = fn ($amount) => 'Rp'.number_format(((int) $amount) / 1000000000, 2, ',', '.').' M';
    @endphp
                                ...
                                <div class="mt-6 grid gap-6 lg:grid-cols-[1.12fr_.88fr]">
                                    <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                                        ...
                                        <div class="mt-6 grid gap-x-6 gap-y-5 sm:grid-cols-2">
                                            @forelse ($populationStats['jobs'] as $job)
                                                <div>
                                                    <div class="flex items-center justify-between gap-4">
                                            ...
                                            @empty
                                                <p class="text-sm font-semibold text-slate-500 sm:col-span-2">Belum ada data pekerjaan.</p>
                                            @endforelse
                                        </div>
                                    </article>
                                    ...
                                    <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                                        ...
                                        <div class="mt-6 space-y-5">
                                            @forelse ($populationStats['religions'] as $religion)
                                                <div>
                                                    <div class="flex items-center justify-between gap-4">
                                            ...
                                            @empty
                                                <p class="text-sm font-semibold text-slate-500">Belum ada data agama.</p>
                                            @endforelse
                                        </div>
                                    </article>
                                </div>
                                    ...
                                    @if ($apbdesStats['hasData'])
                                        <div>
                                            ...
                                            <div class="mt-5">
                                                @foreach ($apbdesYears as $year)
                                                    @php
                                                        $surplusDefisit = ($year['income'] + $year['financingIncome']) - ($year['spending'] + $year['financingExpense']);
                                                    @endphp
                                                    <div class="{{ $loop->last ? '' : 'hidden' }}" data-apbdes-year-panel="{{ $year['year'] }}">
                                                        <div class="grid gap-5 md:grid-cols-2">
                                                            <article class="rounded border border-slate-200 bg-white px-5 py-4 shadow-sm">
                                                                <div class="flex items-center gap-3 text-slate-600"><span class="h-3 w-3 rounded-full bg-slate-400"></span><p class="text-lg font-bold">Pendapatan</p></div>
                                                                <p class="mt-3 font-display text-3xl font-bold text-slate-700">{{ $formatRupiah($year['income']) }}</p>
                                                            </article>
                                                            <article class="rounded border border-slate-200 bg-white px-5 py-4 shadow-sm">
                                                                <div class="flex items-center gap-3 text-slate-600"><span class="h-3 w-3 rounded-full bg-slate-400"></span><p class="text-lg font-bold">Belanja</p></div>
                                                        ...
                                                        <article class="mt-5 overflow-hidden rounded border border-slate-200 bg-white shadow-sm">
                                                            <div class="border-b border-slate-200 px-5 py-4"><p class="text-lg font-bold text-slate-600">Pembiayaan</p></div>
                                                            <div class="grid md:grid-cols-2">
                                                                <div class="px-5 py-4 md:border-r md:border-slate-200">
                                                                    <div class="flex items-center gap-3 text-slate-600"><span class="h-3 w-3 rounded-full bg-slate-400"></span><p class="text-lg font-bold">Penerimaan</p></div>
                                                                ...
                                                                <div class="border-t border-slate-200 px-5 py-4 md:border-t-0">
                                                                    <div class="flex items-center gap-3 text-slate-600"><span class="h-3 w-3 rounded-full bg-slate-400"></span><p class="text-lg font-bold">Pengeluaran</p></div>
                                                        ...
                                                        <article class="mt-5 rounded border border-slate-200 bg-white px-5 py-6 text-center shadow-lg shadow-slate-200/50">
                                                            <p class="text-xl font-bold text-slate-600">Surplus/Defisit <span class="ml-2 font-display text-3xl text-slate-700">{{ $formatRupiah($surplusDefisit) }}</span></p>
                                                        </article>
                                                    </div>
                                                @endforeach
                                            </div>
