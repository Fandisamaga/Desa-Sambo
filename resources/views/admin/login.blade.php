...
<html lang="id">
    ...
    <body class="grid min-h-screen place-items-center bg-emerald-50 p-5">
        <main class="w-full max-w-md rounded-3xl bg-white p-8 shadow-xl shadow-emerald-950/10 ring-1 ring-emerald-900/5">
            ...
            @if ($errors->any())
                <p class="mt-5 rounded-xl bg-red-50 p-3 text-sm text-red-700">{{ $errors->first() }}</p>
            @endif
            ...
            <form method="POST" action="{{ route('admin.login.store') }}" class="mt-7 space-y-5">
                ...
                <button class="btn-primary w-full justify-center" type="submit">Masuk dashboard →</button>
            </form>

            <a class="mt-6 block text-center text-sm font-bold text-emerald-700" href="{{ route('home') }}">← Kembali ke website publik</a>
        </main>
    </body>
</html>
