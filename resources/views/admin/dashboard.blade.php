...
@section('content')
    <div class="grid gap-3 min-[360px]:grid-cols-2 sm:gap-5 sm:grid-cols-2 xl:grid-cols-4">
        @foreach ($stats as $stat)
            <a href="{{ route($stat['route']) }}" class="rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md sm:p-5">
                <p class="text-sm font-medium text-slate-500">{{ $stat['label'] }}</p>
                <p class="mt-2 font-display text-2xl font-bold text-slate-900 sm:mt-3 sm:text-3xl">{{ number_format((int) $stat['value'], 0, ',', '.') }}</p>
    ...
    <div class="mt-8">
        <section class="rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-200 sm:p-6">
